...
# ============================ OPENAPI ENDPOINTS =============================
#
#   Method   Endpoint                                   Implemented   Works
# ----------------------------------------------------------------------------
#   GET      /health                                    yes           yes
...
#   POST     /artifact/{artifact_type}                  yes           yes
#   GET      /artifact/model/{id}/rate                  yes           no
#   GET      /artifact/{artifact_type}/{id}/cost        yes           no
#   POST     /authenticate                              yes           yes
#   GET      /artifact/byName/{name}                    no            no
#   GET      /artifact/{artifact_type}/{id}/audit       no            no
#   GET      /artifact/model/{id}/lineage               yes           no
#   POST     /artifact/model/{id}/license-check         yes           no
#   POST     /artifact/byRegEx                          yes           no
#   DELETE   /reset                                     yes           yes
#   GET      /tracks                                    yes           yes
#
# ============================================================================
...
def reset_registry():
    ...
    # TODO(rybkr): Check permissions for 401 response
    ...
    try:
        ...
        if len(password) == len(DEFAULT_PASSWORD):
            for i, (c1, c2) in enumerate(zip(password, DEFAULT_PASSWORD)):
                if c1 != c2:
                    logger.warning(
                        f"First mismatch at position {i}: got {repr(c1)}, expected {repr(c2)}"
                    )
                    break
        ...
        if not password_valid:
            ...
            logger.warning(
                f"Password mismatch. Expected length: {len(DEFAULT_PASSWORD)}, Got length: {len(password)}"
            )
