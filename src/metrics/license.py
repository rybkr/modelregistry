    ...
    def compute(self, model: Model) -> None:
        ...
        try:
            ...
            if readme:
                ...
                license_keywords = [
                    "license", "apache", "mit", "bsd", "gpl", "lgpl", "cc", "creative commons",
                    "apache-2.0", "mit license", "bsd license", "gpl-3", "lgpl-2.1",
                    "licensed under", "license: mit", "license: apache"
                ]
                has_license_keywords = any(keyword in readme_lower for keyword in license_keywords)
                
                # Log for debugging - use info level so it's always visible
                logger.info(f"License keyword check: has_license_keywords={has_license_keywords}, readme_length={len(readme)}")
                if has_license_keywords:
                    # Log which keywords were found
                    found_keywords = [kw for kw in license_keywords if kw in readme_lower]
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"License API returned non-numeric score: {api_score_raw}, treating as 0.0")
                        api_score = 0.0
                    
                    logger.info(f"License API returned score: {api_score}, min_score from keywords: {min_score}")
                    ...
                    if final_score > api_score:
                        ...
                        result["justification"] = result.get("justification", "") + f" (Adjusted: license keywords found, minimum {min_score})"
                        logger.info(f"License score adjusted from {api_score} to {final_score} based on keywords")
                    else:
                        logger.info(f"License score using API value {api_score} (>= min_score {min_score})")
                    
                    # Always set the value explicitly
                    self.value = float(final_score)
                    self.details = {"model": result}
                    logger.info(f"License metric final value set to: {self.value} (type: {type(self.value)})")
                except Exception as api_error:
                    # If API fails, use keyword-based score
                    logger.warning(f"License API call failed: {api_error}, using keyword-based score {min_score}")
                    ...
                    if has_license_keywords:
                        ...
                        logger.warning(f"License API failed but license mentioned in README, using keyword-based score {min_score}")
                    else:
                        self.value = 0.0
                        self.details = {
                            "model": {
            ...
            try:
                if model.model is not None:
                    ...
                    if readme:
                        ...
                        if has_license_keywords:
                            self.value = 0.5
                            self.details = {"error": str(e), "recovered": "Keyword-based score after exception"}
                            logger.warning(f"License metric exception recovered with keyword-based score 0.5")
                            self.latency_ms = int((time.perf_counter() - t0) * 1000)
                            return
            except Exception:
