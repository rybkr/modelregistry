...
def is_safe_regex(pattern: str) -> bool:
    ...
    if len(pattern) > 100:
        logger.warning(
            f"Regex pattern rejected: pattern too long ({len(pattern)} > 100)"
        )
        return False
    ...
    for danger_pattern in dangerous_patterns:
        match = regex_validate_with_timeout(
            danger_pattern, pattern, timeout_seconds=0.1
        )
        if match is None:
            logger.warning(
                f"Regex pattern validation timed out or failed for pattern: {pattern}"
            )
            return False
        if match:
            logger.warning(
                f"Regex pattern rejected: dangerous pattern detected ({danger_pattern})"
            )
            return False
...
def regex_validate_with_timeout(
    pattern_str: str, text: str, timeout_seconds: float = 0.1
) -> Optional[re.Match]:
    ...
    def search_packages(self, query: str, use_regex: bool = False) -> List[Package]:
        ...
        if use_regex:
            ...
            if not is_safe_regex(query):
                logger.warning(
                    f"Regex search rejected: pattern failed safety validation: {query}"
                )
                return []
            ...
            try:
                ...
                logger.warning(
                    f"Regex pattern compilation failed: {query}, error: {str(e)}"
                )
                return []
    ...
    def get_artifacts_by_query(
        ...
        with self._lock:
            ...
            if enumerate_all:
                ...
                # For exact name matching: when there's a single query, return only ONE result
                ...
                for query in queries:
                    ...
                    # Filter by name - use EXACT matching (case-sensitive)
                    if query_name and query_name != "*":
                        # Sort packages by ID for deterministic ordering when multiple matches exist
                        # This ensures we always return the same artifact when multiple have the same name
                        sorted_packages = sorted(
                            self.packages.values(), key=lambda p: p.id
                        )
                        # For each query with a specific name, find the first exact match
                        for pkg in sorted_packages:
                            # Exact name match (case-sensitive)
                            if query_name == pkg.name:
                                # Filter by types if specified in this query
                                if query_types:
                                    url = pkg.metadata.get("url", "")
                                    pkg_type = "model"
                                    if "huggingface.co/datasets/" in url:
                                    ...
                                    if pkg_type not in query_types:
                                        continue
                                # Only add the first exact match for this query
                                # This ensures each query returns at most ONE result
                                if pkg not in matching_packages:
                                    matching_packages.append(pkg)
                                    # If single query, stop immediately after finding one match
                                    if single_query:
                                        break
                                    # Break after finding first match for this query
                                    break
                        # If single query and we found a match, stop processing
                        if single_query and matching_packages:
                            break
                    elif query_types:
                        # Filter by types only (when name is "*" or empty)
                        # For type-only queries, add all matching packages
